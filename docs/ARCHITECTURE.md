...
各サービスは Cloud Run から Cloud SQL の private IP に VPC 経由で接続する。接続元は同じ VPC 内に閉じているため、VPC をまたぐ経路は持たない。
...
Cloud Run ジョブには `env/modules/foundation/job-monitoring` を当て、失敗したタスク試行を検知する。ジョブは要求を受けず 5xx も応答時間も出さないため、サービスとは別の指標で監視する。ジョブ名はジョブを定義するモジュールの `output` から受け取り、監視の宛先が実体とずれないようにする。

ERROR ログの監視には Cloud Logging が標準で出す件数メトリクスを severity で絞って使う。ログベースメトリクスを定義する方式と違い、ログバケットもシンクも要らず、アラートポリシーだけで完結する。

要求の失敗は 5xx の監視に任せ、ERROR ログは単発では発報しない閾値にする。同じ障害が 5xx と ERROR ログの両方に現れ、どちらも単発で発報させると 1 件の障害で 2 通の通知が届くため。Cloud Monitoring はポリシーをまたいでインシデントをまとめる仕組みを持たず、複数の条件を 1 つのポリシーに入れても時系列ごとにインシデントが開くので、統合では解決できない。

dev も監視対象に含める。通知の届かない環境を作らないため。テストで踏んだエラーが通知を埋めないよう、閾値は環境ごとに変える。
...
Cloud SQL には `env/modules/foundation/database-monitoring` を当て、CPU・メモリ・ディスクの使用率と同時接続数を監視する。インスタンス名はジョブ監視と同じく、インスタンスを定義するモジュールの `output` から受け取る。

使用率の閾値は環境で変えない。資源の逼迫は環境によらず同じ割合で危険なため。同時接続数だけ環境ごとに置くのは、上限がマシンタイプで変わるため。

インスタンスの起動状態は監視しない。夜間停止でコストを抑える運用のため停止が正常な状態であり、発報させると通知が埋もれる。
